...
const STEPS: StepState[] = [
  {
    title: "单工作区的痛苦",
    desc: "两个任务都在进行中，但两份改动会落到同一目录并相互冲突。",
    op: "task_create x2",
    tasks: [
      { id: 1, subject: "鉴权重构", status: "in_progress", worktree: "" },
      { id: 2, subject: "登录界面打磨", status: "in_progress", worktree: "" },
    ],
    worktrees: [],
    lanes: [
      { name: "main", files: ["auth/service.java", "ui/Login.tsx"], highlight: true },
  ...
  {
    title: "为任务 1 分配车道",
    desc: "先建一条 worktree 车道，再把它与任务 1 关联，归属清清楚楚。",
    op: "worktree_create(name='auth-refactor', task_id=1)",
    tasks: [
      { id: 1, subject: "鉴权重构", status: "in_progress", worktree: "auth-refactor" },
      { id: 2, subject: "登录界面打磨", status: "in_progress", worktree: "" },
    ],
    worktrees: [
    ...
    lanes: [
      ...
      { name: "wt/auth-refactor", files: ["auth/service.java"], highlight: true },
      { name: "wt/ui-login", files: [] },
    ],
  },
  {
    title: "为任务 2 分配车道",
    desc: "车道创建和任务关联可以解耦。这里先建车道，再把任务 2 绑上去。",
    ...
    lanes: [
      { name: "main", files: [] },
      { name: "wt/auth-refactor", files: ["auth/service.java"] },
      { name: "wt/ui-login", files: ["ui/Login.tsx"], highlight: true },
    ],
  },
  {
    title: "在隔离车道里执行命令",
    desc: "每条命令按选中的车道目录路由，而不是按共享根目录。",
    op: "worktree_run('auth-refactor', 'mvn test -Dtest=AuthTest -q')",
    tasks: [
      { id: 1, subject: "鉴权重构", status: "in_progress", worktree: "auth-refactor" },
      { id: 2, subject: "登录界面打磨", status: "in_progress", worktree: "ui-login" },
    ],
    ...
    lanes: [
      { name: "main", files: [] },
      { name: "wt/auth-refactor", files: ["auth/service.java", "test/AuthTest.java"], highlight: true },
      { name: "wt/ui-login", files: ["ui/Login.tsx", "ui/Login.css"] },
    ],
  },
  {
    title: "保留一条车道，关闭另一条",
    desc: "收尾可以混搭决策：保留 ui-login 继续跟进，移除 auth-refactor 并完成任务 1。",
    op: "worktree_keep('ui-login')\nworktree_remove('auth-refactor', complete_task=true)\nworktree_events(limit=10)",
    tasks: [
      { id: 1, subject: "鉴权重构", status: "completed", worktree: "" },
      { id: 2, subject: "登录界面打磨", status: "in_progress", worktree: "ui-login" },
    ],
    worktrees: [
  ...
  {
    title: "隔离 + 协调 + 事件",
    desc: "看板维护共享真相，worktree 车道隔离执行，事件流提供可审计的旁路追踪。",
...
const WORKTREE_STATE_LABEL: Record<WorktreeRow["state"], string> = {
  none: "未创建",
  active: "活跃",
  kept: "已保留",
  removed: "已移除",
          ...
          <div className="rounded-md border border-zinc-200 dark:border-zinc-700">
            ...
            <div className="space-y-2 p-2">
              ...
              {step.worktrees.map((wt) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-[11px] font-semibold text-zinc-800 dark:text-zinc-100">{wt.name}</span>
                    <span className="rounded bg-white/70 px-1.5 py-0.5 text-[10px] font-semibold text-zinc-600 dark:bg-zinc-950/30 dark:text-zinc-300">
                      {WORKTREE_STATE_LABEL[wt.state]}
                    </span>
                  </div>
