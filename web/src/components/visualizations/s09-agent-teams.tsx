...
// Step configuration
const STEPS = [
  { title: "团队", desc: "团队采用 leader-worker 模式。每个队友都有基于文件的邮箱 inbox。" },
  { title: "组长分配工作", desc: "通信是异步的：向接收者的 .jsonl inbox 文件写入消息。" },
  { title: "读取收件箱", desc: "队友在每次 LLM 调用前轮询 inbox；新消息会进入上下文。" },
  { title: "独立运行", desc: "每个队友独立运行自己的 agent loop。" },
  { title: "传递结果", desc: "结果沿同一邮箱机制流转；所有通信都通过文件完成。" },
  { title: "反馈回路", desc: "邮箱模式支持任意通信拓扑：线性、广播、轮询。" },
  { title: "文件化协调", desc: "没有共享内存，没有锁；所有协调通过仅追加文件完成：简单、稳健、可调试。" },
];
