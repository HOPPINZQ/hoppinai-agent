...
// Step annotations
const STEP_INFO = [
  { title: "分发映射", desc: "把工具名、处理函数做映射，使用AI返回的结构化数据反射调用对应的处理函数，循环代码本身永远不变。" },
  { title: "路由：bash", desc: "tool_call.name -> handlers['bash'](input)。基于名称的路由。" },
  { title: "路由：read_file", desc: "同样的模式，不同的处理器：校验输入、执行、返 回结果。" },
  { title: "路由：write_file", desc: "每个工具都会返回一个 tool_result，并回写到 messages[] 里。" },
  { title: "路由：edit_file", desc: "新增一个工具 = 新增一个映射。" },
  { title: "关键洞察", desc: "while 循环保持不变；你只是在扩展工具，仅此而已。" },
];
