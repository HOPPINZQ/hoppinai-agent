...
const PARENT_BASE_MESSAGES: MessageBlock[] = [
  { id: "p1", label: "user: 帮我测试一下AuthController的所有接口", color: "bg-blue-500" },
  { id: "p2", label: "assistant: 计划构建中...", color: "bg-zinc-600" },
  { id: "p3", label: "tool_result: 先看看代码，再编写测试用例", color: "bg-emerald-500" },
];

const TASK_PROMPT: MessageBlock = {
  id: "task",
  label: "task: 查看代码并构建测试用例",
  color: "bg-purple-500",
};

const CHILD_WORK_MESSAGES: MessageBlock[] = [
  { id: "c1", label: "tool_use: 读取 AuthController", color: "bg-amber-500" },
  { id: "c2", label: "tool_use: 编写 AuthTest", color: "bg-amber-500" },
];

const SUMMARY_BLOCK: MessageBlock = {
  id: "summary",
  label: "总结: 3个方法的测试用例都生成了，测试都通过！",
  color: "bg-teal-500",
};
...
const STEPS = [
  ...
  },
  {
    title: "生成子代理",
    description:
      "Task 工具创建一个拥有全新 messages[] 的子代理，只传递任务描述。",
  },
  {
    title: "独立工作",
    description:
      "子代理拥有自己的上下文，不会看到父代理的历史。",
  },
  {
    title: "压缩结果",
    description:
  ...
  },
  {
    title: "上下文保持清爽",
    description:
      "父代理得到一段干净的摘要而不膨胀上下文：这就是通过 messages[] 实现的新鲜上下文隔离。",
  },
];
