...
import { VERSION_META, VERSION_ORDER, LEARNING_PATH } from "@/lib/constants";
...
function detectLocale(relPath: string): "zh"  {
...
function main() {
  ...
  if (fs.existsSync(DOCS_DIR)) {
    ...
    for (const locale of localeDirs) {
      ...
      for (const filename of docFiles) {
        ...
        docs.push({ version, locale: locale as "zh", title, content });
      }
    }
  ...
  // 5.5. Post-process docs: replace content that references files
  // e.g., "s01.md" -> content of s1.md (note: s01.md references s1.md)
  const processedDocs: DocContent[] = [];
  let replacedCount = 0;

  for (const doc of docs) {
    let processedContent = doc.content;

    // Check if content is a simple filename reference (no newlines, looks like *.md)
    if (processedContent.match(/^\w+\.md$/)) {
      const filename = processedContent;

      // Map s01.md -> s1.md, s02.md -> s2.md, etc.
      const mappedFilename = filename.replace(/^s(\d+)(a|b|c)?\.md$/, "s$1$2.md");

      const sourcePath = path.join(OUT_DIR, mappedFilename);
  ...
  if (replacedCount > 0) {
    console.log(`  Replaced ${replacedCount} file references with actual content`);
  }
