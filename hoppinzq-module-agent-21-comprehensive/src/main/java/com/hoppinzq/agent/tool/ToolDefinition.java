...
public static ToolDefinition ListFilesDefinition = new ToolDefinition(
    ...
    public static ToolDefinition CreateTaskDefinition = new ToolDefinition(
            "create_task",
            "创建一个新任务。任务会被 teammates 的自主认领机制（IdlePoller）自动发现并认领。",
            createInputSchema(
                    Map.of(
                            "subject", createProperty("string", "任务标题。"),
                            "description", createProperty("string", "任务描述。")
                    ),
                    List.of("subject")
            ),
    ...
    public static ToolDefinition ListTasksDefinition = new ToolDefinition(
            "list_tasks",
            "列出所有任务（含状态和依赖）。",
    ...
    public static ToolDefinition GetTaskDefinition = new ToolDefinition(
            "get_task",
            "获取单个任务详情。",
            createInputSchema(
                    Map.of("taskId", createProperty("integer", "任务 id。")),
                    List.of("taskId")
            ),
            TaskGetInput.class,
            Tools::getTask
    ...
    public static ToolDefinition ClaimTaskDefinition = new ToolDefinition(
            "claim_task",
            "认领一个任务（设置 owner + in_progress 状态）。仅 teammate 可调用。",
            createInputSchema(
                    Map.of("taskId", createProperty("integer", "要认领的任务 id。")),
                    List.of("taskId")
            ),
    ...
    public static ToolDefinition CompleteTaskDefinition = new ToolDefinition(
            "complete_task",
            "将任务标记为完成。",
            createInputSchema(
                    Map.of("taskId", createProperty("integer", "要完成的任务 id。")),
                    List.of("taskId")
            ),
            TaskClaimInput.class,
            Tools::completeTask
    ...
    public static ToolDefinition CreateWorktreeDefinition = new ToolDefinition(
            "create_worktree",
            "创建一个 git worktree（分支 wt/<name> 挂到 .worktrees/<name> 目录），并可选绑定到某个 task。" +
                    "teammate 认领带 worktree 的 task 后会自动把 cwd 切到该目录。",
            createInputSchema(
                    Map.of(
                            "name", createProperty("string", "worktree 名字（也是分支名 wt/<name>）"),
                            "taskId", createProperty("integer", "可选：要绑定的 task id")
                    ),
                    List.of("name")
            ),
            CreateWorktreeInput.class,
            Tools::createWorktree
    );

    public static ToolDefinition RemoveWorktreeDefinition = new ToolDefinition(
            "remove_worktree",
            "移除一个 worktree。discard=true（默认）时连同分支一起删除；discard=false 仅从 git worktree 索引中移除。",
            createInputSchema(
                    Map.of(
                            "name", createProperty("string", "worktree 名字"),
                            "discard", createProperty("boolean", "是否连同分支删除，默认 true")
                    ),
                    List.of("name")
            ),
            RemoveWorktreeInput.class,
            Tools::removeWorktree
    );

    public static ToolDefinition KeepWorktreeDefinition = new ToolDefinition(
            "keep_worktree",
            "标记某个 worktree 为 keep：完成后不自动清理。教学版仅记录日志，实际清理由 remove_worktree 显式调用。",
            createInputSchema(
                    Map.of("name", createProperty("string", "worktree 名字")),
                    List.of("name")
            ),
            KeepWorktreeInput.class,
            Tools::keepWorktree
    );

    // ========================= mock MCP 工具 =========================

    public static ToolDefinition ConnectMcpDefinition = new ToolDefinition(
            "connect_mcp",
            "连接一个 mock MCP server（教学版，进程内 mock）。可选 server：docs（文档搜索）、deploy（部署）。" +
                    "连接后该 server 的工具按 mcp__<server>__<tool> 前缀暴露给 LLM。",
            createInputSchema(
                    Map.of("server", createProperty("string", "要连接的 server 名：docs / deploy")),
                    List.of("server")
            ),
            ConnectMcpInput.class,
            Tools::connectMcp
    );

    // 下面是 connect 之后可用的 mock MCP 工具（前缀 mcp__<server>__<tool>，避免命名冲突）
    public static ToolDefinition McpDocsSearchDefinition = new ToolDefinition(
            "mcp__docs__search",
            "(readOnly) docs server：在 mock 文档库里按 query 搜索。",
            createInputSchema(
                    Map.of("query", createProperty("string", "搜索关键词")),
                    List.of("query")
            ),
            String.class,
            (java.util.function.Function<String, String>) input -> Tools.mcpCall("mcp__docs__search", input)
    );

    public static ToolDefinition McpDocsGetVersionDefinition = new ToolDefinition(
            "mcp__docs__get_version",
            "(readOnly) docs server：返回 mock 文档库版本号。",
            createInputSchema(Map.of(), List.of()),
            String.class,
            (java.util.function.Function<String, String>) input -> Tools.mcpCall("mcp__docs__get_version", input)
    );

    public static ToolDefinition McpDeployStatusDefinition = new ToolDefinition(
            "mcp__deploy__status",
            "(readOnly) deploy server：查询当前部署状态。",
            createInputSchema(Map.of(), List.of()),
            String.class,
            (java.util.function.Function<String, String>) input -> Tools.mcpCall("mcp__deploy__status", input)
    );

    public static ToolDefinition McpDeployRunDefinition = new ToolDefinition(
            "mcp__deploy__run",
            "(destructive) deploy server：触发一次部署。需要 env 字段。",
            createInputSchema(
                    Map.of("env", createProperty("string", "目标环境，如 staging / production")),
                    List.of("env")
            ),
            String.class,
            (java.util.function.Function<String, String>) input -> Tools.mcpCall("mcp__deploy__run", input)
    );
