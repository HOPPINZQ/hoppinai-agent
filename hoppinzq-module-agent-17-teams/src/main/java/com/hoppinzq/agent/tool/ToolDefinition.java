...
public static ToolDefinition ListFilesDefinition = new ToolDefinition(
    ...
    public static ToolDefinition SpawnTeammateDefinition = new ToolDefinition(
            "spawn_teammate",
            "启动一个 teammate agent，让它在独立的守护线程里自主完成一项任务。\n" +
                    "teammate 拥有自己的对话上下文和 bash/read_file/write_file/send_message 工具，\n" +
                    "和 lead 之间通过文件邮箱（<ROOT>/.mailboxes/*.jsonl）异步通信。\n" +
                    "spawn 后立即返回，不要等待 teammate 结束；后续用 check_inbox 读取 teammate 的回执。",
            createInputSchema(
                    Map.of(
                            "name", createProperty("string", "teammate 的名字（也是其邮箱名），例如 'alice'。"),
                            "role", createProperty("string", "teammate 的角色描述，例如 '后端开发'。"),
                            "prompt", createProperty("string", "派给 teammate 的初始任务说明，应足够具体。")
                    ),
    ...
    public static ToolDefinition SendMessageDefinition = new ToolDefinition(
            "send_message",
            "向某个 teammate（或 lead）的邮箱发送一条消息。lead 调用时 from 自动填 'lead'，" +
                    "teammate 在自己的线程里调用时 from 自动填其名字。",
            createInputSchema(
                    Map.of(
                            "to", createProperty("string", "收件人名字（teammate 名字或 'lead'）。"),
                            "content", createProperty("string", "消息内容。")
                    ),
                    List.of("to", "content")
            ),
    ...
    public static ToolDefinition CheckInboxDefinition = new ToolDefinition(
            "check_inbox",
            "读取 lead 自己的收件箱，返回所有未读消息（读取即消费，同一条消息不会重复出现）。",
            createInputSchema(
                    Map.of(),
                    List.of()
            ),
