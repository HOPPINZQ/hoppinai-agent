    ...
    private String buildSystemPrompt() {
        if (REACT_ENABLE) {
            ...
            return String.format("""
                ...
                1. 遇到不熟悉的领域时，主动使用 load_skill 工具获取专业知识
                2. 遇到不熟悉的编程概念、需要遵循特定最佳实践、执行标准化工作流程时加载技能
                3. 流程：识别知识缺口 → 使用 load_skill 加载 → 阅读理解 → 应用知识解决问题
                ...
                """, actionNames, toolDescriptions, skillLoader.getDescriptions(), ROOT, System.getProperty("os.name"));
        } else {
            return """
                # 角色
