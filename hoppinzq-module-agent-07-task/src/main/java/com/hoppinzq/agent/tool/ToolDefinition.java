...
public class ToolDefinition {
    ...
    public static ToolDefinition SkillsDefinition = new ToolDefinition(
            "load_skill",
            "加载指定的技能（工具集合）。可用技能：" + String.join(", ", Agent07.skillLoader.getAvailableSkills()),
            createInputSchema(
    ...
    public static ToolDefinition TodoDefinition = new ToolDefinition(
            ...
            TodoInput.class,
            Agent07.todoManager::updateTodos
    );
    ...
    public static ToolDefinition TaskCreateDefinition = new ToolDefinition(
            ...
            "• blockedBy: 此任务依赖的前置任务ID列表（可选，支持短ID前缀匹配）",
            createInputSchema(
                    Map.of(
                            ...
                            "blockedBy", Map.of(
                                    ...
                                    "description", "此任务依赖的前置任务ID列表（支持短ID，如 'a1b2c3d4'）。这些任务完成后，当前任务才能开始执行。"
                            )
                    ),
                    List.of("subject")
            ),
            ...
            createInputSchema(
                    Map.of(
                            ...
                            "addBlockedBy", Map.of(
                                    "type", "array",
                                    "items", Map.of("type", "string"),
                                    "description", "添加此任务依赖的前置任务ID列表（支持短ID）。这些任务完成后，当前任务才能开始执行。"
                            ),
                            "addBlocks", Map.of(
                                    "type", "array",
                                    "items", Map.of("type", "string"),
                                    "description", "添加依赖于此任务的后置任务ID列表（支持短ID）。当前任务完成后，这些任务才能开始执行。"
                            )
                    ),
                    List.of("taskId")
            ),
