...
/**
 * ToolDefinition 工具定义类（CLI 版本）。
 * 与 web 模块版本的区别：静态字段引用的 manager 来自 {@link CliAgent}，而非 WebZQAgent。
 *
 * @author hoppinzq
 */
@Data
...
public static ToolDefinition ListFilesDefinition = new ToolDefinition(
    ...
    public static ToolDefinition SkillsDefinition = new ToolDefinition(
            "load_skill",
            "加载指定的技能（工具集合）。可用技能：" + String.join(", ", CliAgent.skillLoader.getAvailableSkills()),
            createInputSchema(
    ...
    public static ToolDefinition TodoDefinition = new ToolDefinition(
            ...
            TodoInput.class,
            CliAgent.todoManager::updateTodos
    );
